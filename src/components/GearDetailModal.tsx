...
import { getGearImage } from '../constants/gearCategoryImages';
...
const GearDetailModal: React.FC<GearDetailModalProps> = ({
  ...
  // Reset all form/modal state when a different gear item is opened
  useEffect(() => {
    if (gear) {
      setEditing(false);
      setSaving(false);
  ...
  const gearImageSrc = getGearImage(gear.image_url ?? null, gear.category);
