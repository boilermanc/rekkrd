      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-th-bg px-4">
          <div className="glass-morphism rounded-2xl p-8 max-w-md w-full text-center space-y-6">
            ...
            {this.state.error && (
              <pre className="text-xs text-red-400/70 bg-th-bg/40 rounded-lg p-3 overflow-auto max-h-32 text-left">
                {this.state.error.message}
              </pre>
            )}
            <button
              onClick={() => window.location.reload()}
              className="px-6 py-2.5 bg-[#c45a30] hover:bg-[#dd6e42] text-th-text rounded-full text-sm font-medium transition-colors"
            >
              Reload App
            </button>
          </div>
